...
var MultiLanguage = function () {
  ...
  _createClass(MultiLanguage, [{
    ...
    value: function init(_languages, _vue) {
      ...
      var languageKeys = Object.keys(this.languages);

      if (!userLang) userLang = languageKeys[0];

      this.userLang = userLang;

      if (languageKeys.includes(localStorage.getItem('vue-lang'))) this.userLang = localStorage.getItem('vue-lang');
    ...
    value: function matchLanguage(lang) {
      ...
      Object.keys(this.languages).forEach(function (path) {
        if (lang.toLowerCase() === path.toLowerCase()) match = path;
      });
